...
## 14. Update log (July 2026 - cloud demo + mobile + product design)

- **Cloud demo hardening**: Demo Mode defaults ON; CSV writes tolerate read-only hosting (`save_csv`/`save_feedback` return True/False instead of raising); public-demo privacy disclaimer; Feedback form always succeeds with a "may not persist" note when storage is unavailable.
- **Mobile navigation**: Streamlit header is no longer hidden (it holds the sidebar reopen arrow); synced in-page "📱 App menu" expander (Demo Mode toggle + page selector + AI pill) mirrors the sidebar via `st.session_state["current_page"]` / `["demo_mode"]` masters with per-widget keys and on_change sync.
- **Product design pass**: `.streamlit/config.toml` theme (emerald primary - no more red default widgets); hero landing card ("Take control of your spending"); custom stat-card grid with icons + helper text (2-across on phones) replacing `st.metric` on Dashboard/Coach snapshots; icon + badge action cards; checklist-style recommended actions; card-row ranked spending list; goal cards with 🎯 + deadline badge on the Savings Goals page; education concept cards in the Learning Hub; intro cards on Add/Coach/Goals/Hub/Feedback pages; calmer mobile heading scale. All saving-opportunity logic, debt special-casing and wording rules unchanged.
